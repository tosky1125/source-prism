...
#[tokio::test]
async fn repo_test_runs_returns_empty_local_state_without_database()
-> Result<(), Box<dyn std::error::Error>> {
    let app = app(AppState::for_test_repo_path(".".into())?);
    let request = Request::builder()
        .method(Method::GET)
        .uri("/v1/repos/local/test-runs")
        .body(Body::empty())?;
    ...
    assert_eq!(body.pointer("/run_count").and_then(Value::as_u64), Some(0));
    assert_eq!(
        body.pointer("/runs").and_then(Value::as_array),
        Some(&Vec::new())
    );
    Ok(())
}
