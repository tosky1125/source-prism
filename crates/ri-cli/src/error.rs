...
pub(crate) enum CliError {
    #[error(
        "usage: ri-cli config check --env-file <path> | db migrate | repo manifest --repo <path> | index --repo <path> --sha <sha> | symbols --repo <path> | changed-symbols --diff <diff> | references --symbol <symbol> | architecture --repo <path> | impact --symbol <symbol> | search-context <query> | test-context --symbol <symbol> | tests import-junit --repo <path> --sha <sha> --junit <file> | tests import-lcov --repo <path> --sha <sha> --lcov <file> | tests import-cobertura --repo <path> --sha <sha> --cobertura <file> | search sync --once | search drift-check [--expect-mismatch fixture] | search rebuild --from-postgres"
    )]
    Usage,
