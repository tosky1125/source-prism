...
use ri_behavior::parse_pytest_json;
...
pub(crate) async fn import(
    ...
    let parsed = ImportPytestArgs::parse(&mut args)?;
    ...
    let body = fs::read_to_string(&parsed.pytest_path)?;
    let report = parse_pytest_json(&body)?;
    ...
    let generation = generation_store
        .begin_generation(
            ...
            &commit_sha,
            "test_results",
            Some("ri-cli-pytest-json-v1"),
        )
        .await?;
    let outcome = PgTestRunStore::new(pool)
        .replace_pytest_run_for_generation(
            &generation.generation_id,
            parsed.pytest_path.to_string_lossy().as_ref(),
            &report,
        )
        .await?;
    generation_store
...
#[derive(Debug)]
struct ImportPytestArgs {
    repo_path: PathBuf,
    sha: String,
    pytest_path: PathBuf,
}

impl ImportPytestArgs {
    fn parse(args: &mut impl Iterator<Item = String>) -> Result<Self, CliError> {
        let mut repo_path = None;
        let mut sha = None;
        let mut pytest_path = None;
        while let Some(flag) = args.next() {
            match flag.as_str() {
                "--repo" => repo_path = args.next().map(PathBuf::from),
                "--sha" => sha = args.next(),
                "--pytest-json" => pytest_path = args.next().map(PathBuf::from),
        ...
        Ok(Self {
            ...
            pytest_path: pytest_path.ok_or(CliError::Usage)?,
        })
    }
}
