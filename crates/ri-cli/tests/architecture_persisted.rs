...
#[tokio::test]
async fn architecture_command_uses_persisted_repo_index() -> TestResult {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_ri-cli"))
        .current_dir(repo_root)
        .env("DATABASE_URL", database_url.as_str())
        .args(["architecture", "--repo-id", repo_id])
        .output()?;
...
impl TempRepo {
    fn create() -> TestResult<Self> {
        ...
        let path = std::env::temp_dir().join(format!("source-prism-architecture-db-{suffix}"));
