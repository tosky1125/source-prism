...
#[tokio::test]
async fn tests_import_cobertura_persists_coverage_segments()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    repo.write_file(
        "coverage.xml",
        r#"<coverage><packages><package><classes><class filename="src/lib.rs"><lines><line number="1" hits="1"/></lines></class></classes></package></packages></coverage>"#,
    )?;
...
impl TempRepo {
    fn create() -> Result<Self, Box<dyn std::error::Error>> {
        let suffix = SystemTime::now()
            ...
            .as_nanos()
            .to_string();
        let path = std::env::temp_dir().join(format!("source-prism-cli-cobertura-{suffix}"));
...
async fn active_segment_count(pool: &PgPool, repo_id: &str) -> Result<i64, sqlx::Error> {
    let row = sqlx::query(
        "SELECT count(*)::bigint AS count FROM coverage_segments WHERE repo_id = $1 AND format = 'cobertura' AND stale_at IS NULL",
    )
    .bind(repo_id)
    .fetch_one(pool)
    .await?;
