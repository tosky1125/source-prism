...
#[tokio::test]
async fn tests_import_lcov_persists_coverage_segments() -> Result<(), Box<dyn std::error::Error>> {
    ...
    repo.commit()?;
    repo.write_file("lcov.info", "SF:src/lib.rs\nDA:1,1\nend_of_record\n")?;
...
impl TempRepo {
    fn create() -> Result<Self, Box<dyn std::error::Error>> {
        let suffix = SystemTime::now()
            ...
            .as_nanos()
            .to_string();
        let path = std::env::temp_dir().join(format!("source-prism-cli-coverage-{suffix}"));
...
async fn active_segment_count(pool: &PgPool, repo_id: &str) -> Result<i64, sqlx::Error> {
    let row = sqlx::query(
        "SELECT count(*)::bigint AS count FROM coverage_segments WHERE repo_id = $1 AND stale_at IS NULL",
    )
    .bind(repo_id)
    .fetch_one(pool)
    .await?;
