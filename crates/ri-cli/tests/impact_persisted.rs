...
#[tokio::test]
async fn impact_command_uses_persisted_repo_index() -> TestResult {
...
",
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_ri-cli"))
        .current_dir(repo_root)
        .env("DATABASE_URL", database_url.as_str())
        .args(["impact", "--repo-id", repo_id, "--symbol", "apply_tax"])
        .output()?;
    ...
    assert_json_array_contains(&body, "/direct_callers", "apply_tax_adds_rate")?;
...
impl TempRepo {
    fn create() -> TestResult<Self> {
        ...
        let path = std::env::temp_dir().join(format!("source-prism-impact-db-{suffix}"));
