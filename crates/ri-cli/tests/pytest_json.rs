...
#[tokio::test]
async fn tests_import_pytest_json_persists_test_run_results()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    repo.write_file(
        "pytest.json",
        r#"{"tests":[{"nodeid":"tests/test_invoice.py::test_adds_rate","outcome":"passed","duration":0.001}]}"#,
    )?;
    ...
    assert_eq!(active_pytest_result_count(&pool, repo_id).await?, 1);
...
impl TempRepo {
    fn create() -> Result<Self, Box<dyn std::error::Error>> {
        let suffix = SystemTime::now()
            ...
            .as_nanos()
            .to_string();
        let path = std::env::temp_dir().join(format!("source-prism-cli-pytest-{suffix}"));
...
async fn active_pytest_result_count(pool: &PgPool, repo_id: &str) -> Result<i64, sqlx::Error> {
    let row = sqlx::query(
        "SELECT count(*)::bigint AS count FROM test_results results JOIN test_runs runs ON results.test_run_id = runs.test_run_id WHERE results.repo_id = $1 AND runs.framework = 'pytest' AND results.stale_at IS NULL",
    )
    .bind(repo_id)
    .fetch_one(pool)
    .await?;
