...
#[test]
fn references_command_uses_repo_path_argument() -> Result<(), Box<dyn std::error::Error>> {
...
impl TempRepo {
    fn create() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let path = std::env::temp_dir().join(format!("source-prism-references-{suffix}"));
