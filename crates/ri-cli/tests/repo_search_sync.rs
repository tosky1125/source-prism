...
#[tokio::test]
async fn repo_search_sync_command_reports_latest_generation_queue_state() -> TestResult {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_ri-cli"))
        ...
        .args(["repo-search-sync", "--repo-id", fixture.repo_id.as_str()])
        .output()?;
...
impl Fixture {
    async fn create(pool: &PgPool) -> TestResult<Self> {
        ...
        let fixture = Self {
            repo_id: format!("cli-repo-search-sync-{suffix}"),
    ...
    async fn cleanup(&self, pool: &PgPool) -> Result<(), sqlx::Error> {
        sqlx::query(
            ...
            WHERE job_id IN (
                SELECT job_id FROM jobs WHERE generation_id = $1
            )
            ",
        )
        .bind(&self.generation_id)
        .execute(pool)
        .await?;
        sqlx::query("DELETE FROM jobs WHERE generation_id = $1")
            .bind(&self.generation_id)
            .execute(pool)
            .await?;
