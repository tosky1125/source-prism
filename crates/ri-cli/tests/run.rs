...
#[tokio::test]
async fn run_command_returns_one_run_with_evidence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    repo.write_file(
        "src/lib.rs",
        r"
pub fn cli_run_fixture() -> i32 {
    7
}

#[test]
fn cli_run_fixture_is_indexed() {
    assert_eq!(cli_run_fixture(), 7);
}
",
    )?;
    repo.commit()?;
    ...
    let body = serde_json::from_slice::<Value>(&run_output.stdout)?;
    assert_eq!(body.pointer("/kind").and_then(Value::as_str), Some("run"));
    assert_eq!(
        body.pointer("/run/run_id").and_then(Value::as_str),
        Some(run_id)
    );
    assert_eq!(
    ...
    assert_eq!(
        body.pointer("/run/evidence/search_sync_job_details/0/state")
            .and_then(Value::as_str),
        Some("queued")
    );
    assert_eq!(
        body.pointer("/run/evidence/search_sync_job_details/0/attempts/0/status")
            .and_then(Value::as_str),
        Some("started")
    );
    assert_eq!(
        body.pointer("/run/evidence/search_sync_outbox_details/0/state")
            .and_then(Value::as_str),
        Some("queued")
    );
    assert_count_at_least(
        &body,
        "/run/evidence/search_sync_outbox_state_counts/queued",
        1,
    )?;
    assert_count_at_least(&body, "/run/evidence/symbols", 2)?;
...
impl TempRepo {
    fn create() -> Result<Self, Box<dyn std::error::Error>> {
        let path = std::env::temp_dir().join(format!("source-prism-cli-run-{}", unique_suffix()?));
