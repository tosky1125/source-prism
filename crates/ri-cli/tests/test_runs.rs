...
#[tokio::test]
async fn tests_import_junit_persists_test_run_results() -> Result<(), Box<dyn std::error::Error>> {
    ...
    repo.write_file(
        "src/lib.rs",
        "pub fn apply_tax(value: i32) -> i32 { value + 1 }\n",
    )?;
    repo.commit()?;
    let junit = repo.path().join("junit.xml");
    fs::write(
        &junit,
    ...
    assert_eq!(active_result_count(&pool, repo_id).await?, 1);
...
impl TempRepo {
    fn create() -> Result<Self, Box<dyn std::error::Error>> {
        let suffix = SystemTime::now()
            ...
            .as_nanos()
            .to_string();
        let path = std::env::temp_dir().join(format!("source-prism-cli-test-runs-{suffix}"));
...
async fn active_result_count(pool: &PgPool, repo_id: &str) -> Result<i64, sqlx::Error> {
    let row = sqlx::query(
        "SELECT count(*)::bigint AS count FROM test_results WHERE repo_id = $1 AND stale_at IS NULL",
    )
    .bind(repo_id)
    .fetch_one(pool)
    .await?;
