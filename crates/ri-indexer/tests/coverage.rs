...
#[tokio::test]
async fn active_coverage_preserves_jacoco_format_for_repo() -> Result<(), Box<dyn std::error::Error>>
{
    let Some(pool) = test_pool().await? else {
        return Ok(());
    };
    ...
    let store = PgCoverageStore::new(pool.clone());
    let outcome = store
        .replace_jacoco_for_generation(&generation.generation_id, "jacoco.xml", &report)
        .await?;
    PgGenerationStore::new(pool.clone())
        .finish_generation(&generation.generation_id)
        .await?;
    ...
    let segment = segments
        .first()
        .ok_or_else(|| std::io::Error::other("missing coverage segment"))?;
    assert_eq!(segment.format, "jacoco");
    fixture.cleanup(&pool).await?;
    Ok(())
}
