...
#[tokio::test]
async fn active_test_runs_preserves_go_test_framework_for_repo()
-> Result<(), Box<dyn std::error::Error>> {
    let Some(pool) = test_pool().await? else {
        return Ok(());
    };
    ...
    let generation = PgGenerationStore::new(pool.clone())
        ...
        .await?;
    let report = parse_junit_xml(
        r#"<testsuite name="go_test"><testcase classname="example.com/invoice" name="TestAddsRate"/></testsuite>"#,
    )?;
    PgTestRunStore::new(pool.clone())
        .replace_go_test_run_for_generation(&generation.generation_id, "go-test.json", &report)
        .await?;
    PgGenerationStore::new(pool.clone())
        .finish_generation(&generation.generation_id)
        .await?;
    ...
    let run = runs
        .first()
        .ok_or_else(|| std::io::Error::other("missing test run"))?;
    assert_eq!(run.framework, "go_test");
    assert_eq!(run.results.len(), 1);
    fixture.cleanup(&pool).await?;
    Ok(())
}
